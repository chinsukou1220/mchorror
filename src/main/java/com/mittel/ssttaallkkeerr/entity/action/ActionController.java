...
public class ActionController extends Behavior<HorrorSteveEntity> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, HorrorSteveEntity owner) {
        ...
        if (owner.hasBeenSeenSinceWarp) {
            ...
            int redNightCount = com.mittel.ssttaallkkeerr.world.RedNightState.get(level).weaknessLevel;
            if (redNightCount > 0) {
            ...
            if (players.isPresent() && !players.get().isEmpty()) {
                isUnderground = com.mittel.ssttaallkkeerr.util.UndergroundDetector.isPlayerUnderground(level, players.get().get(0));
            }
            ...
            if (isUnderground || isNether) {
                ...
                // ==========================================
                    ...
                    if (Math.random() < 0.00005 * multiplier) {
                        if (players.isPresent() && !players.get().isEmpty()) {
                            if (com.mittel.ssttaallkkeerr.util.PlayerBlockTracker.hasLargeCluster(players.get().get(0).getUUID(), 30)) {
