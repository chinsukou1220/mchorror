package com.mittel.ssttaallkkeerr;
...
import com.mittel.ssttaallkkeerr.entity.sensor.GlobalPlayerSensor;
import net.minecraft.world.level.levelgen.Heightmap;
import net.minecraft.sounds.SoundEvent;
import net.minecraft.world.level.block.Block;
import net.minecraft.world.level.block.state.BlockBehaviour;
import net.minecraft.world.item.BlockItem;
import com.mittel.ssttaallkkeerr.block.GhostBlock;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import net.fabricmc.fabric.api.command.v2.CommandRegistrationCallback;
import com.mittel.ssttaallkkeerr.command.HorrorDebugCommand;
import com.mittel.ssttaallkkeerr.item.HorrorDebugItem;
...
public class SsttaallkkeerrMod implements ModInitializer {
    ...
    public static final net.minecraft.world.level.levelgen.feature.Feature<NoneFeatureConfiguration> RANDOM_BLOCKS = 
        new com.mittel.ssttaallkkeerr.world.feature.RandomBlockFeature(NoneFeatureConfiguration.CODEC);
	...
	public static final EntityType<com.mittel.ssttaallkkeerr.entity.HorrorSteveEntity> HORROR_STEVE = Registry.register(
			BuiltInRegistries.ENTITY_TYPE,
			new ResourceLocation(MOD_ID, "horror_steve"),
			FabricEntityTypeBuilder.create(MobCategory.MISC, com.mittel.ssttaallkkeerr.entity.HorrorSteveEntity::new)
					.dimensions(EntityDimensions.fixed(0.6F, 1.95F)).build()
	);

	public static final EntityType<com.mittel.ssttaallkkeerr.entity.FinalActionSteveEntity> FINAL_ACTION_STEVE = Registry.register(
			BuiltInRegistries.ENTITY_TYPE,
			new ResourceLocation(MOD_ID, "finalactionsteve"),
			FabricEntityTypeBuilder.create(MobCategory.MISC, com.mittel.ssttaallkkeerr.entity.FinalActionSteveEntity::new)
					.dimensions(EntityDimensions.fixed(0.6f, 1.95f)).build()
	);
	...
	public static final Item HOUSE_DEBUG = Registry.register(
			...
			new com.mittel.ssttaallkkeerr.item.HouseDebugItem(new Item.Properties().stacksTo(1))
	);
	...
	public void onInitialize() {
		...
		com.mittel.ssttaallkkeerr.util.WorldBackupManager.checkPendingRestore();

		// Initialize First Action (10-minute event)
		FirstActionManager.init();

		// Register Entity attributes
		FabricDefaultAttributeRegistry.register(HORROR_STEVE, com.mittel.ssttaallkkeerr.entity.HorrorSteveEntity.createAttributes());
		FabricDefaultAttributeRegistry.register(FINAL_ACTION_STEVE, com.mittel.ssttaallkkeerr.entity.FinalActionSteveEntity.createAttributes());
		...
		net.fabricmc.fabric.api.entity.event.v1.ServerPlayerEvents.AFTER_RESPAWN.register((oldPlayer, newPlayer, alive) -> {
			if (pendingNightmare.contains(newPlayer.getUUID())) {
				...
				for (com.mittel.ssttaallkkeerr.entity.HorrorSteveEntity steve : currentLevel.getEntitiesOfClass(com.mittel.ssttaallkkeerr.entity.HorrorSteveEntity.class, newPlayer.getBoundingBox().inflate(100000.0))) {
					steve.discard();
				}
				...
				if (nightmareLevel != null) {
					...
					// ナイトメア用のタイマーをリセット
					com.mittel.ssttaallkkeerr.SsttaallkkeerrMod.nightmareTicks.put(newPlayer.getUUID(), 0);
				}
			}
		});
		...
		net.fabricmc.fabric.api.entity.event.v1.ServerEntityWorldChangeEvents.AFTER_PLAYER_CHANGE_WORLD.register((player, origin, destination) -> {
			com.mittel.ssttaallkkeerr.FirstActionManager.onPlayerChangeDimension(player, origin, destination);
		});
		...
		net.fabricmc.fabric.api.event.lifecycle.v1.ServerTickEvents.END_WORLD_TICK.register(level -> {
			if (level.dimension() == net.minecraft.world.level.Level.OVERWORLD) {
				com.mittel.ssttaallkkeerr.world.RedNightManager.tick(level);
			}
		});
		...
		net.fabricmc.fabric.api.event.lifecycle.v1.ServerTickEvents.END_SERVER_TICK.register(server -> {
			...
			if (nightmareLevel != null) {
				for (net.minecraft.server.level.ServerPlayer player : nightmareLevel.players()) {
					// マップから滞在時間を取得
					int ticks = com.mittel.ssttaallkkeerr.SsttaallkkeerrMod.nightmareTicks.getOrDefault(player.getUUID(), 0);
					ticks++;
					com.mittel.ssttaallkkeerr.SsttaallkkeerrMod.nightmareTicks.put(player.getUUID(), ticks);
					...
					if (ticks == 10) { // ディメンション読み込み直後（0.5秒後）にスポーン
						...
						for (int i = 0; i < count; i++) {
							...
							com.mittel.ssttaallkkeerr.entity.FinalActionSteveEntity steve = SsttaallkkeerrMod.FINAL_ACTION_STEVE.create(nightmareLevel);
							if (steve != null) {
					...
					if (ticks == 400) {
					    ...
					    com.mittel.ssttaallkkeerr.util.WorldBackupManager.setPendingRestore(server);
		...
		net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STARTING.register(server -> {
			com.mittel.ssttaallkkeerr.util.WorldBackupManager.checkAndBackup(server);
		});
	}
}